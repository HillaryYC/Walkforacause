...
- Set `APP_URL`, mail, and database credentials for your hosting environment.
- Cache config, routes, and views:
  - `php artisan config:cache`
  - `php artisan route:cache`
  - `php artisan view:cache`
- Run migrations with `php artisan migrate --force`.
- Run the queue worker (Supervisor/systemd recommended):
...
- Schedule runs daily at `BACKUP_TIME` (default `02:00`).
- Configure retention with `BACKUP_RETENTION_DAYS`.
